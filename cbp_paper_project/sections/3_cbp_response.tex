...
The CBP was aligned with the StarDICE telescope to improve the focus. The StarDICE camera was set close to infinity with no filter. We adjusted the CBP focus to get the smallest extension of the pinhole image. A slight offset was then added to account for the small change in focus introduced by tightening the locking screws. The focus was to check to resist slot changes and even complete removal and reinstallation of the pinhole.
...
In our analysis, the spectrograph data are used to monitor the laser wavelength, but also the laser stray light contamination. Its main sources are a \SI{1064}{\nm} component from the Yagg laser pump and a \SI{532}{\nm} half harmonic. Therefore, we had to calibrate it in wavelength and flux. 
