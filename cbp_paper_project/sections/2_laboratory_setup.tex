...
Grism \MARC{Describe the GRISM}, and a \SI{0.2}{\milli\meter}
...
batoid \cite{} raytracing software is available at \MARC{release} and
is used to build a model of the gathered data.  \MARC{At this stage we
  need a precise measurement of the
  \begin{itemize}
  \item Primary diameter
  \item Secondary/primary distance
  \item Secondary diameter
  \item Secondary/filter distance
  \item Filter/ccd distance
  \item Window/CCD distance
  \end{itemize}
}
...
of \MARC{work out, from jeremy's first plot}, negligible for the
...
To determine the effect of temperature on QE, the QE of the solar cell was measured as it was heated. At longer wavelengths, the QE increased slightly as the temperature increased (see Fig.~\ref{fig:SC_temp}). This trend is consistent with the temperature dependence of silicon's QE (cite this paper). At 1050 nm, the QE changes by more than 0.1 percent per degree.
\begin{figure}[!h]
\centering
...
There is another issue linked to the choice of pinhole in the CBP slide. When shooting into the \SD telescope, we use the \spinhole pinhole so it is seen as a pointlike source by the \SD camera. But when shooting into the solar cell, we use the \bpinhole pinhole to have the best signal to noise ratio. Since $\Rcbp(\lambda)$ depends on the pinhole diameter, we need to intercalibrate the two responses $R_\mathrm{CBP}^{\mathrm{\SI{5}{\milli\meter}}} (\lambda)$ and $R_\mathrm{CBP}^{\mathrm{\SI{75}{\micro\meter}}} (\lambda)$. This intercalibration is done with the measurements in the line No.~8 of Table~\ref{tab:schedule}. \\
...
\begin{table*}[t]{}
    ...
    \begin{tabular}{M{.25cm} M{2.25cm} M{1.75cm} M{1.1cm} M{1.75cm} M{3cm} M{3.5cm} M{1.25cm}}
        \hline\hline
         ...
         2 & Radial pupil stitching & StarDice & \SI{75}{\micro\meter} & MAX & \shortstack{u, g, r, i, z, y, \\ EMPTY, GRATING} & 4 radial positions on the mirror  & 1 per position \\
         
         3 & Quadrant pupil stitching & StarDice & \SI{75}{\micro\meter} & MAX & EMPTY & 4 quadrant positions on the mirror  & 1 per position \\
         
         4 & Repeatability measurement & StarDice & \SI{75}{\micro\meter} & MAX & \shortstack{u, g, r, i, z, y, \\ EMPTY, GRATING} & Same position on the mirror and focal plane for \SI{75}{\micro\meter} pinhole & 3 \\
         ...
         6 & StarDice main calibration & StarDice & \SI{5}{\milli\meter} & MAX & \shortstack{u, g, r, i, z, y, \\ EMPTY, GRATING} & Same position on the mirror and focal plane for \SI{5}{\milli\meter} pinhole & 5 \\
