...
The telescope itself is not part of the CBP design, which is thought \emph{a priori} to measure any telescope. As in \cite{Mondrik_2023}, we restrict here the experiment to the controled environment of the laboratory, leaving to further development the practical questions specific to in situ measurements.
...
In this paper, the design evolved from that described in \cite{Mondrik_2023} in many details and three salient points: \begin{itemize}
\item The laser light source loaned from NIST has been replaced by an EKSPLA NT 252 with a slightly different wavelength power cutoff;
\item we add a filtering system to avoid injecting laser harmonics into the light beam; 
\item and most importantly, the calibration and monitoring of the CBP transmission has been included as part of the instrumentation and observation procedure, relying on a solar cell instead of an external measurement in a different experimental setup. 
\end{itemize} 
